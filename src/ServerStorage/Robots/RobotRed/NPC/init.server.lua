...
local player = game:GetService("Players"):GetPlayerByUserId(script.Parent:GetAttribute("PlayerUserId"))

local function patrol()
	while isAlive() do
		local pathStart = script.Parent.HumanoidRootPart.Position
		local pathEnd
		if player then
			local character = player.Character or player.CharacterAdded:Wait()
			local rootPart = character:WaitForChild("HumanoidRootPart")
			pathEnd = getRandomPointInCircle(rootPart.Position, PATROL_RADIUS)
		else
			pathEnd = getRandomPointInCircle(startPosition, PATROL_RADIUS)
		end
